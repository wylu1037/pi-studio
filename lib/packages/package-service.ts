...
function createManager(cwd: string) {
  return createStudioPackageManager(cwd)
}
...
export async function installRuntimePackage(input: {
  ...
  await packageManager.installAndPersist(input.source, { local: false })
  await refreshSessions(input.cwd)
}
...
export async function removeRuntimePackage(input: {
  ...
  const { packageManager } = createManager(input.cwd)
  await packageManager.removeAndPersist(input.source, { local: false })
  const { removePackageSourceFromAgents } = await import('@/lib/db/repository')
  removePackageSourceFromAgents(input.source)
  await refreshSessions(input.cwd)
}
